...
class DDPG(object):
    def __init__(self, a_dim, s_dim):
        ...
        self.ctrain = th.optim.Adam(self.Critic_eval.parameters(),lr=LR_C)
        self.atrain = th.optim.Adam(self.Actor_eval.parameters(),lr=LR_A)
        self.loss_td = nn.MSELoss()

    def choose_action(self, s):
        s = th.unsqueeze(th.FloatTensor(s), 0)
    ...
    def learn(self):
        ...
        bs = th.FloatTensor(bt[:, :self.s_dim])
        ba = th.FloatTensor(bt[:, self.s_dim: self.s_dim + self.a_dim])
        br = th.FloatTensor(bt[:, -self.s_dim - 1: -self.s_dim])
        bs_ = th.FloatTensor(bt[:, -self.s_dim:])
